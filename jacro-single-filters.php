...
if($single_cinema):
    ...
    $result = $wpdb->get_row($wpdb->prepare("SELECT * FROM $table_locations WHERE code = %s", $single_cinema));
    $cinema = $result->id;
    ...
    if($single_loctype =='all'){
        ...
        if(empty($default_selected)){ $defaultclass = ' active'; }
    ?>
    ...
    <div class="row" id="film_section">
        ...
        $post_films = get_films( $selectedType, $cinema, $selectedDate, $order_by_date );
        ...
        foreach($post_films['post'] as $key=>$film_val) {  
            ...
            $post_arr = get_performances( $film_val->code, $selectedDate ,$selectedType, $cinema);
            ...
            if(!empty($post_arr)) {   
                ...
                foreach($post_arr as $val) {
                    ...
                    if(strtotime("$perform_date $start_time") > $strtotimezone) {     
                        ...
                        $datasarray[$perform_date][$filmtitle][$val->id]['cinema_name'] = get_option('term_'.$cinema);
